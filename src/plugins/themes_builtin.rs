...
pub fn parse_defaults_level(override_level: Option<u8>) -> u8 {
    ...
    match env::var("BOXY_DEFAULTS_LEVEL") {
        Ok(value) => match value.parse::<u8>() {
            ...
            Ok(n) if n > 2 => {
                eprintln!(
                    "Warning: BOXY_DEFAULTS_LEVEL={} invalid (>2), using level 2",
                    n
                );
                2
            }
            ...
            Err(_) => {
                eprintln!(
                    "Warning: BOXY_DEFAULTS_LEVEL='{}' invalid (not a number), using level 0",
                    value
                );
                0
            }
...
fn get_minimal_themes() -> (HashMap<String, BoxyTheme>, Vec<String>) {
    ...
    let trail =
        vec!["  📦 Built-in themes (Level 0 - Minimal): default styles, blueprint".to_string()];
...
fn get_standard_themes() -> (HashMap<String, BoxyTheme>, Vec<String>) {
    let mut themes = HashMap::new();
    let trail = vec![
        "  📦 Built-in themes (Level 1 - Standard): error, success, warning, info".to_string(),
    ];
...
fn get_extended_themes() -> (HashMap<String, BoxyTheme>, Vec<String>) {
    ...
    trail[0] =
        "  📦 Built-in themes (Level 2 - Extended): error, success, warning, info + legacy themes"
            .to_string();
